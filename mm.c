...
// This code finds a free block that is a required size then if no block is found then it allocates more memory.
// It then searchs through the free list that gets more blocks.
...
//  This code takes the next_block_tag from the block that is currently being used. Then block_size
//  is used to check the size of an allocated block. Then remove_free_block removes the current block from the free list. 
    next_block_tag = (ptr_free_block->size_and_tags) & TAG_PRECEDING_USED;
    block_size = SIZE(ptr_free_block->size_and_tags);
    remove_free_block(ptr_free_block);

//  This if statement chekcs if the block is needed based on the differece in size. It then sets the size_and_tags
//  for the allocated block. Then it sets the size_and_tags for the leftover free blocks. Finally it inserts the
//  rest of the free blocks in the free list.
    if (block_size - req_size >= MIN_BLOCK_SIZE){
    	ptr_free_block->size_and_tags = req_size | next_block_tag;
...
//  In the case that we do not need to split this updates the block tags. Then it allows the pointer to the next block. 
//  The following_block is then used to set it to the next block. Then ptr_free_block is set to the current block.
...
//  Finally this returns the pointer to an allocated block.
...
// The block_to_free sets the block to be freed by changing the pointer. Then payload_size helps find the size of a block that will 
// be freed. Next following_block gets the pointer to the next block. The next following_block marks the previous block unused. 
// block_to_free is then used to mark the present block as unused. Then size_t updates the tag of the present block. Then 
// insert_free_block is used to insert the free block to the free list. coalesce_free_block is then used to free the block with free// blocks. 
...
//int mm_check() {
//}
  // TODO: Implement a heap consistency checker as needed/desire
