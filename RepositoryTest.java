...
public class RepositoryTest {
    ...
    public void testSynchronize(){
        String[] repo1Message = {"Initial commit."};
        String[] repo2Message = {"Repo2 - First commit"};

        String[] expectedMessage = {"Repo2 - First commit.", "Repo2 - Added fetures X"};

        for(String message : repo1Message){
            repo1.commit(message);
        }
        for(String message : repo1Message){
            repo1.commit(message);
        }
        for(String message : repo2Message){
            repo2.commit(message);
        }
        repo1.synchronize(repo2);
        assertEquals(expectedMessage.length, repo1.getRepoSize());

        String repositoryHistory = repo1.getHistory(expectedMessage.length);
        String[] commit = commits[i];
        String expectedMessage = expectedMessage[i];
        assertTrue(commit.contains(expectedMessage));
    }
}
